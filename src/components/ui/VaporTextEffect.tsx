...
export default function VaporTextEffect({
  texts = [
    "Next.js",
  ...
  // Tracks the "waiting" -> "vaporizing" timeout so we can cancel it
  // if the element scrolls out of view mid-wait.
  ...
  // Start/stop animation cycle based on scroll visibility.
  // Entering view: reset particles + progress to a clean slate, then
  // kick off vaporizing from the start.
  // Leaving view: freeze the animation frame and cancel any pending
  // phase-transition timeout so nothing fires while off-screen.
  // In the isInView effect: don't start vaporizing until particles exist
  useEffect(() => {
    if (isInView) {
      ...
      const tryStart = () => {
        if (particlesRef.current.length > 0) {
          ...
          // particles not built yet — check again next tick
          startAnimationTimeout = setTimeout(tryStart, 50);
        }
      };

      let startAnimationTimeout = setTimeout(tryStart, 100);
 ...
 * threshold: 0.3 means the animation only kicks off once ~30% of the
 * element is actually visible on screen (not just 1px inside the
 * rootMargin buffer), so it reads as "triggered by scroll" rather than
 * firing the instant it's technically in the DOM viewport.
 */
